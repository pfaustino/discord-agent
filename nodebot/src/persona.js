...
export const CAPABILITY_PROMPT = (
  ...
  + 'it should have vocals and lyrics, and clip-vs-full-song if those are still '
  + 'missing — then compose it through Lyria; never pretend a song is being made in '
  + 'text alone. Tracks they like can be saved in a small per-server '
...
export const MUSIC_NOTE = (
  ...
  + '- When someone asks for a song and you have enough to write a prompt '
  + '(genre/style plus mood or subject is enough for a short clip), CALL '
  + 'generate_music. Never say you are writing, crafting, whipping something up, '
  + 'or that a track will be ready soon unless you just called generate_music '
  + 'in this same turn — text alone does not make music.\n'
  + '- If you still need one detail, ask a short real question — do not stall '
  + 'with fake progress updates. On their next answer (or this one if they '
  + 'already gave genre, mood, and subject), call generate_music.\n'
